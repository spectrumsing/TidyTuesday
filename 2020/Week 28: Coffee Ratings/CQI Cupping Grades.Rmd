...
```{r data preperation, message=FALSE}
df_coffee <- 
  data %>% 
...
```{r fig, fig.height=17, fig.width=12}
windowsFonts(inter_title = windowsFont("Inter Semi Bold"), 
...
ggplot(df_coffee, 
  ...
  stat_dots(quantiles = 100,
            ...
            position = "dodge",
            justification = -0.04) +
  stat_pointinterval(aes(interval_color = stat(level)),
                     ...
                     position = "dodge",
                     show.legend = F) +
  scale_color_manual(values = c("#aa9486", "#d67237", "#5b1a18"),
                     aesthetics = "interval_color") +
  geom_text(aes(x = med, y = country_of_origin, label = round(med, 1)),
            size = 4.5,
            vjust = 1.7,
            family = "inter_axis") +
  labs(title = "Coffee Quality Institute Grades by Country",
  ...
  theme(plot.background = element_rect(fill = "#edede9"),
        ...
        plot.caption = element_text(family = "inter_body",
                                    size = 12,
                                    color = "#aa9486",
                                    margin = margin(t = 5, b = 5), hjust = 0.35),
